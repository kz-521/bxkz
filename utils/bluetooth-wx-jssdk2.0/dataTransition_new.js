...
//3A60432A5C01211f291E0F4E0C132825 密钥
//060101012D1A683D48271A18316E471A 原始

let AES = require('./aes.js');
// let ECB = require('./mode-ecb.js');
// let padNopadding = require('./pad-nopadding.js');
...
// 加解密用到的密钥
function aesKeyBytes(encryptionKey) {
  console.log("***encryptionKey: ", encryptionKey);
  console.log("***_encryptionKey: ", _encryptionKey);
  if (_encryptionKey.length == 0) {
    _encryptionKey = encryptionKey; // 把encryptionKey存起来，因为encryptionKey可能为空
  }
  if (encryptionKey) {
    _encryptionKey = encryptionKey;
  }
  // var key_Int = new Int8Array([107, 69, 49, 50, 102, 52, 57, 78, 113, 81, 98, 90, 77, 51, 90, 86]);
  var key_Int = new Int8Array(_encryptionKey);
...
// AES加密
function aesEncrypt(str, encryptionKey) {
  var decArray = hexStrToDecArray(str);
  var wordArray = int8parse(decArray);
  console.log(str);
  console.log(wordArray);
  var encrypted = AES.CryptoJS.AES.encrypt(wordArray, aesKeyBytes(encryptionKey), {
      mode: AES.CryptoJS.mode.ECB,
      padding: AES.CryptoJS.pad.NoPadding
  });
  return wordArrayToHexStr(encrypted.ciphertext.words);
}

// AES解密
function aesDecrypt(str, encryptionKey) {
  var decArray = hexStrToDecArray(str);
  var wordArray = int8parse(decArray);
  var base64Str = AES.CryptoJS.enc.Base64.stringify(wordArray);
  var decrypted = AES.CryptoJS.AES.decrypt(base64Str, aesKeyBytes(encryptionKey), {
      mode: AES.CryptoJS.mode.ECB,
      padding: AES.CryptoJS.pad.NoPadding
  });
...
function int8parse(u8arr) {
  ...
  return AES.CryptoJS.lib.WordArray.create(words, len);
}
...
function wordArrayToHexStr(array) {
  ...
  console.log("++++ :", ret.substr(0, ret.length - 1));
